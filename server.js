    ...
    // if(subscriber_id == cred.subscriber_id && password == cred.password){
        ...
        let query = db.db.query(sql,[name],(err,found)=>{
            ...
            if(found.length !== 0 && found[0].payment_confirmation == 'SUCCESS'){
                res.render('dashboard',{order_id,subscription_amount,id, eblPay, due:'0',status:'Paid', name});
            }else if(found.length !== 0 && found[0].payment_confirmation == 'REQUESTED'){
                res.render('dashboard',{order_id:found[0].order_id,subscription_amount,id,due:subscription_amount,status:'Due', eblPay, name});
            }else{
                res.render('dashboard',{order_id,subscription_amount,id,due:subscription_amount,status:'Due', eblPay, name});
            }
        });
    // }else{
